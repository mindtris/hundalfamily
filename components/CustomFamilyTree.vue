<template>
  ...
  >
    <div
      ref="vueFamilyTree"
      :style="{
        position: enableDrag ? 'relative' : null,
        top: `${position.y}px`,
        left: `${position.x}px`,
      }"
    >
      <CustomFamilyTreeBranch :tree="tree" @card-click="cardClick">
        <template v-slot:card="{ item }">
          <slot name="card" :item="item" />
        </template>
      </CustomFamilyTreeBranch>
    </div>
  </div>
</template>

<script>
import CustomFamilyTreeBranch from "./CustomFamilyTreeBranch.vue";

export default {
  name: "CustomFamilyTree",
  components: {
    CustomFamilyTreeBranch,
  },
  props: {
